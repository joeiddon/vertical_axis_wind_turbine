...
radius = 750;          //distance of wings from centre
height = 2200;          //height of the turbine
wingSF = 170;           //essentially the length of the wing as it is scaling a length of 1
extrudeSlices = 40;   //number of chunks to slice the extusion of the cross section into
helicalTwist = 120;    //angle in degrees for each wing to be extruded by
tilt = 0;             //angle for the wing to tilt in to the center by     (not sure if needed but could be useful...)
sectionHeight = 30;
...
module sectionOfTwist(){
	difference(){
	oneTwist();
	translate([0,0, sectionHeight])
	cylinder(r = 2 * radius, h = height);
	}
}
